...
from constants import (
    ...
    OBSTACLE_STATIC_CHANCE, OBSTACLE_SPEED_RANGE,
    ...
    PSO_RELIEF_WEIGHT, PSO_RUN_WEIGHT,
...
class PondSim:
    ...
    def _make_obs(self):
        for _ in range(NUM_OBSTACLES):
            ...
            if is_static:
                ...
                cp = math.cos(phi)
                vx = speed * cp * math.cos(theta)
                vy = speed * cp * math.sin(theta)
    ...
    def _upd_objs(self):
        ...
        for o in self.objs:
            ...
            if o.kind in ('food', 'probiotic'):
                if o.z < _FLOOR_Z: o.z = min(o.z + SINK_SPEED, _FLOOR_Z)
            elif o.kind in ('fecal', 'dead_fish'):
                if o.z < _FLOOR_Z: o.z = min(o.z + SINK_SPEED_HEAVY, _FLOOR_Z)

            o.on_floor = o.z >= _FLOOR_Z
    ...
    def _transform_pollutant(self, o):
        """Roll what the pollutant becomes when it expires."""
        ...
        if roll < cumul:
            self.hazards.append(Hazard(o.x, o.y, o.z, r,
                'nh3', max_age=NH3_EXPIRE_TIMESTEPS,
                vx=random.uniform(-NH3_AREA_SPEED, NH3_AREA_SPEED),
                vy=random.uniform(-NH3_AREA_SPEED, NH3_AREA_SPEED),
                vz=random.uniform(-NH3_AREA_SPEED * 0.5, NH3_AREA_SPEED * 0.5)))
            return
        ...
        if roll < cumul:
            self.hazards.append(Hazard(o.x, o.y, _FLOOR_Z, r, 'disease',
        ...
        if roll < cumul:
            self.hazards.append(Hazard(o.x, o.y, _FLOOR_Z, r, 'parasite',
                                       max_age=PARASITE_AREA_DECAY, is_floor=True))
            return

        # Both disease + parasite
        cumul += POLLUTANT_TO_BOTH_CHANCE
        if roll < cumul:
            self.hazards.append(Hazard(o.x, o.y, _FLOOR_Z, r, 'disease',
                                       max_age=DISEASE_AREA_DECAY, is_floor=True))
            self.hazards.append(Hazard(o.x, o.y, _FLOOR_Z, r * 0.8, 'parasite',
                                       max_age=PARASITE_AREA_DECAY, is_floor=True))
            return
        ...
        if roll < cumul:
            ...
            oz = _clamp(_FLOOR_Z - d, _WALL, POND_DEPTH - _WALL - d)
            self.obstacles.append(Obstacle(ox, oy, oz, w, h, d, True, 0, 0, 0))
            return

        # else: nothing happens, pollutant just disappears
    ...
    def _vecs(self, f, alive):
        ...
        if not f.has_parasite:
            for obs in self.obstacles:
                ssx,ssy,ssz = obs.nearest_surface_predicted(f.x,f.y,f.z,PSO_OBSTACLE_ANTICIPATION_STEPS)
