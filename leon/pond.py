#!/usr/bin/env python3
"""
pond.py -- PondSim: the aquaculture simulation engine.
"""
...
from constants import (
    ...
    OBSTACLE_MAX_WIDTH, OBSTACLE_MAX_HEIGHT,
...
class PondSim:
    ...
    # ────────────────────────────────────────────────────────────
    # Obstacle generation (new: area/aspect + 10% dim cap + speed range)
    # ────────────────────────────────────────────────────────────
    ...
    def _cp(self, x, y):
        return _clamp(x, _WALL, POND_WIDTH - _WALL), _clamp(y, _WALL, POND_HEIGHT - _WALL)
    ...
    # ────────────────────────────────────────────────────────────
    # Update objects, hazards, obstacles
    # ────────────────────────────────────────────────────────────
        ...
        for f in self.fish:
            ...
            for o in self.objs:
                if not o.alive or o.value <= 0:
                    continue
                if _dist(f.x, f.y, o.x, o.y) > f.body_size + FISH_EAT_RANGE:
                    continue
    ...
    # ────────────────────────────────────────────────────────────
    # PSO movement (with obstacle avoidance + tangential sliding)
    # ────────────────────────────────────────────────────────────
    ...
    def _pso(self):
        ...
        for f in alive:
            ...
            for w, dx, dy in self._vecs(f, alive):
                nvx += w * dx
                nvy += w * dy
            ...
            for obs in self.obstacles:
                if obs.contains(nx, ny):
                    # Find nearest surface point from the proposed position
                    sx, sy = obs.nearest_surface(nx, ny)

                    # Surface normal: direction from surface point outward
                    norm_x, norm_y = _norm(nx - sx, ny - sy)

                    # If fish is deep inside (norm is zero), push from center
                    if abs(norm_x) < 1e-8 and abs(norm_y) < 1e-8:
                        ocx, ocy = obs.center()
                        norm_x, norm_y = _norm(f.x - ocx, f.y - ocy)
                        if abs(norm_x) < 1e-8 and abs(norm_y) < 1e-8:
                            norm_x, norm_y = _norm(random.uniform(-1, 1), random.uniform(-1, 1))

                    # Place fish just outside the surface
                    epsilon = 0.5
                    nx = sx + norm_x * epsilon
                    ny = sy + norm_y * epsilon

                    # Decompose velocity: remove normal component, keep tangential
                    dot = f.vx * norm_x + f.vy * norm_y
                    f.vx -= dot * norm_x
                    f.vy -= dot * norm_y
            ...
            # ── Tangential sliding against walls ──
            if nx < _WALL:
                nx = _WALL
                f.vx = 0.0  # zero perpendicular (x), keep parallel (y)
                if f.has_parasite and random.random() < PARASITE_SCRUB_CHANCE:
                    f.has_parasite = False
            ...
            if ny < _WALL:
                ny = _WALL
                f.vy = 0.0  # zero perpendicular (y), keep parallel (x)
                if f.has_parasite and random.random() < PARASITE_SCRUB_CHANCE:
                    f.has_parasite = False
    ...
    def _vecs(self, f, alive):
        ...
        if not f.has_parasite:
            for obs in self.obstacles:
                # Use predicted position for dynamic obstacles
                sx, sy = obs.nearest_surface_predicted(
                    f.x, f.y, PSO_OBSTACLE_ANTICIPATION_STEPS)
                ...
                if 0 < d < SENSITIVE_DISTANCE:
                    ...
                    # Fish is touching the surface — push away from center
                    ocx, ocy = obs.center()
                    dx, dy = _norm(f.x - ocx, f.y - ocy)
                    vecs.append((PSO_OBSTACLE_WEIGHT, dx, dy))

        # ── cRelief (parasitized fish seek obstacles to scrub) ──
        if f.has_parasite:
            sv = self._scrub(f)
            if sv:
                vecs.append((PSO_RELIEF_WEIGHT, sv[0], sv[1]))

        return vecs

    # ────────────────────────────────────────────────────────────
    # Scrub helper (find nearest obstacle surface for parasite relief)
    # ────────────────────────────────────────────────────────────
    ...
    def _scrub(self, f):
        ...
        wall_targets = []
        if f.x > _WALL:
            wall_targets.append((_WALL, f.y, f.x - _WALL))
        if f.x < POND_WIDTH - _WALL:
            wall_targets.append((POND_WIDTH - _WALL, f.y, POND_WIDTH - _WALL - f.x))
        if f.y > _WALL:
            wall_targets.append((f.x, _WALL, f.y - _WALL))
        if f.y < POND_HEIGHT - _WALL:
            wall_targets.append((f.x, POND_HEIGHT - _WALL, POND_HEIGHT - _WALL - f.y))
        if wall_targets:
            wall_targets.sort(key=lambda t: t[2])
            tx, ty, _ = wall_targets[0]
            return _norm(tx - f.x, ty - f.y)
        return None

    # ────────────────────────────────────────────────────────────
    # Cannibalism (simplified: instant kill)
    # ────────────────────────────────────────────────────────────
                ...
                if random.random() < (f.fullness / f.max_fullness) * FECAL_BASE_CHANCE:
                    ...
                    if not stacked:
                        self.objs.append(DynObj(
                            f.x + random.uniform(-3, 3), f.y + random.uniform(-2, 2),
    ...
    def _death(self):
        for f in self.fish:
            ...
            if f.hp <= 0 or f.oxygen <= 0:
                ...
                if f.is_infected:
                    self.hazards.append(Hazard(f.x, f.y,
