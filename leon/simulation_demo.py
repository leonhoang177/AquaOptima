...
MAX_BUDGET = 100.0                # Max allowed per-cycle cost ($). Ponds exceeding this are rejected by the Gatekeeper.
INITIAL_FISH_POPULATION = 10      # Number of Largemouth Bass at the start of each simulation.
AQUACULTURE_DAYS = 10             # Duration of the aquaculture period in real days. Runtime = this * 24 timesteps.
POND_GENERATIONS = 8             # Number of EA selection rounds. More = better convergence but slower.
RUN_SIMULATIONS = 3               # Number of independent parallel timelines. Best champion is picked across all.
INITIAL_POND_COUNT = 5           # Number of random pond genotypes in the first EA generation.
